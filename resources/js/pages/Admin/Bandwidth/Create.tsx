...
export default function BandwidthCreate() {
    const { data, setData, post, processing, errors } = useForm({
        name_bw: '', rate_down: 0, rate_up: 0, rate_down_unit: 'Mbps', rate_up_unit: 'Mbps', burst: '',
    });
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.bandwidth.store'), { onSuccess: () => toast.success('Bandwidth created') });
    };
    return (
        <MainLayout>
            <div className="space-y-6">
                <div className="flex items-center justify-between"><h2 className="text-2xl font-bold">Add Bandwidth</h2><Link href={route('admin.bandwidth.index')}><Button variant="outline">Back</Button></Link></div>
                <Card>
                    <CardHeader><CardTitle>Bandwidth Information</CardTitle></CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2"><Label>Name *</Label><Input value={data.name_bw} onChange={(e) => setData('name_bw', e.target.value)} />{errors.name_bw && <p className="text-sm text-destructive">{errors.name_bw}</p>}</div>
                            ...
                            <div className="flex gap-2">
                                <Button type="submit" disabled={processing}>{processing ? 'Creating...' : 'Create Bandwidth'}</Button>
